...
def sales_by_region(df: pd.DataFrame) -> pd.DataFrame:
    ...
    #--SOLUTION:groupby region, sum sales+profit, sort by sales descending; assign to `result`
    result = (
...
def sales_by_category(df: pd.DataFrame) -> pd.Series:
    ...
    #--SOLUTION:groupby category & sub_category, sum sales, sort descending; assign to `result`
    result = (
...
def top_products(df: pd.DataFrame, n: int = 10) -> pd.Series:
    ...
    #--SOLUTION:groupby product_name, sum sales, sort descending, take head(n); assign to `result`
    result = (
